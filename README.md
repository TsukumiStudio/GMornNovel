...
`bubble_show/hide`、`fade_out(秒)`、`fade_in(秒)`、`wait`、`wait_submit`、`start_tutorial("種類"[, 引数[, 入力モード]])`、`all_hide`、`bgm`、`bgm_stop`、`bgm_mute`、`se`、
`shorts_show/set/pita/zoom/hide`。`bubble_show` の引数は互換用で、枠の差し替えには使わない。
`chara_show` は淡入中でも次の命令へ進む。`chara_move` は指定秒数の移動を完了してから次の命令へ進む。
...
`{"kind": "tutorial", "tutorial": 種類, "argument": 引数, "input_mode": 入力モード}` を渡す。入力モードは
利用側が解釈する任意の文字列で、省略時は空文字列である。handlerがtrueを返したときは、
